...
class EventViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        var url = "http://cs-server.usc.edu:21828/getjson.php?keyword=\(Keyword.sharedInstance.keyword)&type=event"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.eventlabel.text = itemName[indexPath.row]
        let url = NSURL(string: picurl[indexPath.row])
        let data = NSData(contentsOf: (url as? URL)!)
        cell.eventimg.image = UIImage(data: data as! Data)
