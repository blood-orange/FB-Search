...
class GroupViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        var url = "http://cs-server.usc.edu:21828/getjson.php?keyword=\(Keyword.sharedInstance.keyword)&type=group"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.grouplabel.text = itemName[indexPath.row]
        let url = NSURL(string: picurl[indexPath.row])
        let data = NSData(contentsOf: (url as? URL)!)
        cell.groupimg.image = UIImage(data: data as! Data)
