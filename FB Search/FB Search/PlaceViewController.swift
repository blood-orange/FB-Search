...
class PlaceViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        var url = "http://cs-server.usc.edu:21828/getjson.php?keyword=\(Keyword.sharedInstance.keyword)&type=place&lat=\(Keyword.sharedInstance.lat)&lon=\(Keyword.sharedInstance.lon)"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.placelabel.text = itemName[indexPath.row]
        let url = NSURL(string: picurl[indexPath.row])
        let data = NSData(contentsOf: (url as? URL)!)
        cell.placeimg.image = UIImage(data: data as! Data)
